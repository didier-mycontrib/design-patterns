...
public class SvgGenerateVisitor implements AbstractFigVisitor {
	...
	public void doActionForCercle(Cercle c) {
		...
		svgContent.append(MessageFormat.format(format, c.getXc(),c.getYc(),c.getRayon()));
	}

	@Override
	public void doActionForRectangle(Rectangle r) {
		String format="<rect x=''{0}'' y=''{1}'' width=''{2}'' height=''{3}'' style=''fill:green'' />";		
		svgContent.append(MessageFormat.format(format, r.getX1(),r.getY1(),r.getL(),r.getH()));
	}
	...
	public void doActionForLigne(Ligne l) {
		 ...
		 svgContent.append( MessageFormat.format(format,
						 l.getX1(),l.getY1(),l.getX2(),l.getY2(),l.getColorAsString()));
	}
